...
public class UserLoginServlet extends HttpServlet {
	...
	if(DaoFactory.getUserLoginDaoInstance().UserLogin(User_no, Password,Actor)==true){
	 ...
	 //以下操作是通过查小组成员班级然后找到与其同班的同学最后添加进双主键的toupiao表里去实现一对多投票
		     ...
		     List<Xs>list=DaoFactory.getXsDaoInstance().findByStuNo(Bjxzcy);  //找与成员同一个班并且已提交申请的学生（ 因为pkrdsqbxx表里没有bjcy字段所以只能在xsxx表里找）
			   if(list.size()!=0){
		     ...
		     List<Pkrdsqb>list1=DaoFactory.getPkrdsqbDaoInstance().findbyClassName(ClassName);  //在pkrdsqbxx里找同班的同学
			  if(list1.size()!=0){
				for(int j=0;j<list1.size();j++){
				     ...
				     @SuppressWarnings("unused")
					 Pkrdsqb sqb=list1.get(j);    //调用方法找到的值都赋给全局变量，取pkrdsqbxx表里的本班的同学信息 ，加上bjcy字段添加到toupiao里， 实现一个bjcy对pkrdsqbxx里本班每位同学都有一次投票机会
			...
			if(DaoFactory.getToupiaoDaoInstance().checkchongming(Bjxzcy, Xs)==true){    //先判断toupiao表里是否有与bjcy，xs，重复的数据
				DaoFactory.getToupiaoDaoInstance().addToupiao(tp);   //将在pkrdsqbxx表找到的小组成员与其同班同学添加进toupiao表                    
			}
		 }
			}
			}
			...
			List<Xs>list3=DaoFactory.getXsDaoInstance().findByStuNo(User_no);   //取班级成员的班别只能在xsxx表里取 ，找与组员同一个班的学生
