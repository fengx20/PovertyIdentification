...
<html>
  ...
  <body>
   ...
   <tr style="font-weight:bold;" align="center">
      ...
      <td><%=list.get(i).getTongguo()%></td>
      <td><%=list.get(i).getFandui()%></td>
      <td><%=list.get(i).getQingkuang()%></td>
      <td><%=list.get(i).getZhuangtai()%></td>
      <td width="87">
      <a href="servlet/PkrdsqbServlet?action=<%=12 %>&fullstuNo=<%=list.get(i).getFullstuNo()%>">查看申请表</a>
      </td>
      <td width="125"><a href="servlet/PkrdsqbServlet?action=<%=8 %>&fullstuNo=<%=list.get(i).getFullstuNo()%>" onclick="return confirm('确定提交？')" >提交至辅导员岗</a>
      </td>
      <td width="111">
      <a href="javascript:void(0)" onclick="window.open('view/page/Bjxz/bjxzzztuihui.jsp?fullstuNo=<%=list.get(i).getFullstuNo()%>','_blank','width=740,height=330,top=250,left=350')" style="color:red">退回并填写不通过理由</a>
      </td>
   </tr>
  <%
      }
  ...
  %>
    <tr><td colspan="11">暂无通过的申请！</td></tr>
  <%
    }
  %> 
  </table>
